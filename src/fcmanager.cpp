...
inline const QString FCManager::parseSqlQuery(const QString &query) const
{
    ...
    if (query.contains("INSERT", Qt::CaseInsensitive))
    {
        ...
        auto keyList = match_it.next().captured(0).remove(QRegExp("[( ')]")).split(QString(", "), Qt::SkipEmptyParts);

        auto valueList = match_it.hasNext() ?
                match_it.next().captured(0).remove(QRegExp("[( ')]")).split(QString(", "), Qt::SkipEmptyParts) : QStringList();
